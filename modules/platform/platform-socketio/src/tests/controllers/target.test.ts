import { decoratorsModule as decorators, injectionModule as injection, Mapping } from '@methodus/server';
import { MethodResult, AuthType } from '@methodus/framework-commons';
/**
 * @hidden
 */
...
export class TestTarget {
    ...
    public async create(@Mapping.Files('files') files: any, @Mapping.Cookies('cookies') cookies: any, @Mapping.Body('name') name: string): Promise<MethodResult> {
        return new MethodResult({});
    }

    @decorators.MethodMock({})
    @decorators.Method('Get', '/api/player')
    public async list(@Mapping.Headers('auth') auth: string, @Mapping.Query('order_by') orderBy: string): Promise<MethodResult> {
        return new MethodResult({});
    }
    ...
    public async getByField(@Mapping.Param('field') field: string, @Mapping.Param('value') value: number): Promise<MethodResult> {
        return new MethodResult({});
    }
