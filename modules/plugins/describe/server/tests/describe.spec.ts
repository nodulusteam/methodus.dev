...
describe('Test Xserver configuration', () => {
    ...
    const describeView = injection.Injector.resolve(DescribeView);
    const testController = injection.Injector.resolve(TestController);
    const testTarget = injection.Injector.resolve(TestTarget);
    const copyController = injection.Injector.resolve(CopyController);
    let server;
    beforeAll(() => {
