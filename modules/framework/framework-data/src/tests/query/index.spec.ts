...
import { DBHandler, Query, ReturnType } from '../../';
...
describe('pluck', () => {
    ...
    it('multiple fields', async () => {
        await Company.insert({ name: 'Maxim', id: 'Maxim', toBeIgnored: 1, toBeIgnored2: 2 });
        ...
        expect(firstResult.id).to.be.equal('Maxim');
    });
});
...
describe('check the following methods: between, and, or, paging, filter', () => {
    ...
    it('get all alerts between id 100020011 to id 100020020 with severity critical or medium with paging of 5 records', async () => {
        ...
        const query = new Query(Alert)
            .between('id', '100020011', '100020020')
            .and({
                severity: 'critical',
            })
            .or({
    ...
    it('filter all Case by company id and status id not close', async () => {
        let cases = casesData();
        await Case.insert(cases);
        let query = new Query(Case).filter({ _company_id: 'POC' }).and({}).notIn('status_id', ['59d1c3bb6d74f228cc778c9a', '59d1c3bb6d74f228cc778c9b']).count('total_cases');
