...
describe('repo tests - by inheritance', () => {
    const alertMock = {};
    alertMock._id = new mongodb_1.ObjectID();
    /*before(async () => {
    ...
    it('should read record successfully by id from alerts collection', async () => {
        ...
        // console.log(all);
        //given
        const savedAlert = await alert_1.Alert.insert({
            _id: alertMock._id,
            alert_title: 'my_title'
        });
        //when
        const alert = await alert_1.Alert.get(savedAlert.id.toString());
        //then
        chai_1.expect(alert._id.toString()).is.equal(alertMock._id.toString());
    });
    it('repo insert test', async () => {
        // given
        const alertModel = new alert_1.Alert({
            _id: alertMock._id,
            alert_title: 'my_title'
        });
        // when
        const alert = await alertModel.insert();
        //then
        chai_1.expect(alert._id.toString()).is.equal(alertMock._id.toString());
    });
    it('repo save test', async () => {
        ...
        // when
        const alert = await alert_1.Alert.save(data);
        //then
        chai_1.expect(alert._id.toString()).is.equal(alertMock._id.toString());
    });
    it('repo find test', async () => {
        ...
        //when        
        const alertModel = new alert_1.Alert();
        const alert = await alertModel.find({ _id: alertMock._id }, __1.ReturnType.Single);
        //then
        chai_1.expect(alert._id.toString()).is.equal(alertMock._id.toString());
    });
    it('repo find test static', async () => {
        ...
        //given
        //when
        const alert = await alert_1.Alert.find({ _id: alertMock._id }, __1.ReturnType.Single);
        //then
        chai_1.expect(alert._id.toString()).is.equal(alertMock._id.toString());
    });
    it('repo updateMany', async () => {
        ...
        // given
        // when
        const res = await alert_1.Alert.updateMany({ _id: alertMock._id }, { alert_title: 'zokzok_title' });
        // then
        chai_1.expect(res.result.ok).is.equal(1);
    });
    it('uniqBy lodash', async () => {
        ...
        _.map(_.uniq(_.map(obj, function (obj) {
